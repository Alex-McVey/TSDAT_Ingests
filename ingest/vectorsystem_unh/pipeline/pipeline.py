...
class Pipeline(IngestPipeline):
    """--------------------------------------------------------------------------------
    MODAQ GROUP VECTORSYSTEM INGESTION PIPELINE
    ...
    def hook_generate_and_persist_plots(self, dataset: xr.Dataset):
        ...
        with plt.style.context(style_file):
            ...
            filename = DSUtil.get_plot_filename(dataset, "water_temp", "png")
            with self.storage._tmp.get_temp_filepath(filename) as tmp_path:
                fig, ax = plt.subplots(figsize=(10, 8), constrained_layout=True)
                fig.suptitle(
                    f"Water Temperature Readings from {start_date} to {final_date}")
